...
export function RSVPSection() {
  ...
  const stats: RSVPStatsType = useMemo(() => {
    ...
    const notAttending = rsvpData.filter(
      (item) => item.attendance === "Tidak Hadir"
    );
    const totalGuests = rsvpData.reduce(
      (acc, item) => acc + item.guestCount,
      0
    );
  ...
  const filteredData = useMemo(() => {
    return rsvpData.filter((item) => {
      ...
      if (
        filters.invitationId !== "all" &&
        item.invitationId !== filters.invitationId
      ) {
        return false;
      }
      ...
      <CardHeader>
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <CardTitle>Dashboard & RSVP</CardTitle>
            <CardDescription>
