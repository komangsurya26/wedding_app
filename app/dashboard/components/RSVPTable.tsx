...
export function RSVPTable({ data, loading, onViewMessage }: RSVPTableProps) {
    ...
    return (
      <div className="rounded-md border p-8">
        <div className="flex flex-col items-center justify-center text-center text-muted-foreground">
          ...
          <p className="text-sm">
            Data RSVP akan muncul di sini ketika tamu mengisi form konfirmasi
            kehadiran.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="rounded-md border h-[43vh] overflow-y-auto hide-scrollbar">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="w-12">No</TableHead>
          ...
          {data.map((item, index) => (
            <TableRow key={item.id}>
              ...
              <TableCell>
                <Badge
                  variant={
                    item.attendance === "Hadir" ? "default" : "destructive"
                  }
                  className={
